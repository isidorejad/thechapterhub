...
class AuthController {
    ...
    async login(req, res) {
        try {
            ...
            // Re-fetch user with tokens for the response after login,
            // as this is the primary way the frontend AuthContext gets user data.
            // const userWithTokens = await User.findById(user._id)
            //                                  .select('-password')
            //                                  .populate('tokens') // Populate user's token balance
            //                                  .lean();

            // const userResponse = userWithTokens;
            ...
            if (userTokens) {
                userResponse.tokens = {
                    ...
                    // Add other UserToken fields if needed
                };
            } else {
                // If no UserToken entry exists (shouldn't happen with default creation, but good to handle)
                userResponse.tokens = { balance: 0.00 }; // Or a default empty object
            }
            res.json({
                message: 'Login successful!',
                user: userResponse, // Send the populated user object
                token
            });
        } catch (error) {
    ...
    async verifyEmail(req, res) {
        try {
            ...
            // // Re-fetch user with tokens to ensure frontend context is fully updated
            // const userWithTokens = await User.findById(user._id)
            //                                  .select('-password')
            //                                  .populate('tokens')
            //                                  .lean();
            ...
            //     delete userResponse.tokens.user;
            // }
              // Re-fetch user without populating 'tokens'
